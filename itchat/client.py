...
class client(object):
    ...
    def auto_login(self):
        def open_QR():
            for get_count in range(10):
                ...
                while not self.get_QRTest(): time.sleep(1)
        ...
        out.print_line('Login successfully as %s\n' % self.storageClass.memberList, False)
        out.print_line('Login successfully as %s\n' % self.storageClass.userName, False)
        out.print_line('Login successfully as %s\n' % self.storageClass.chatroomList, False)
    ...
    def get_QR(self, uuid=None):
        ...
        # @author leeChen
        # modify function getQR()
        # do not open QR image
        # there  maybe cause a bug by image Name

    def get_QRTest(self, uuid=None):
        try:
            if uuid == None: uuid = self.uuid
            url = '%s/qrcode/%s' % (BASE_URL, uuid)
            r = self.s.get(url, stream=True)
            ###BUG####
            ###QR_DIR = '/home/master/PycharmProjects/WeChatSend/static/QR.jpg'
            QR_DIR = '/home/master/PycharmProjects/WeChatSend/static/'+uuid+'.jpg'
            with open(QR_DIR, 'wb') as f:
                f.write(r.content)
            return True
        except:
    ...
    def get_QRTest2(self, uuid=None):
        try:
            if uuid == None: uuid = self.uuid
            url = '%s/qrcode/%s' % (BASE_URL, uuid)
            r = self.s.get(url, stream=True)
    ...
    def check_login(self, uuid=None):
        ...
        if data and data.group(1) == '200':
            try:
                ...
                print " No such file or directory: '/home/master/PycharmProjects/WeChatSend/static/"
            #os.remove(QR.jpg)
    ...
    def get_contractTest(self, update=False):
        print "====begin==========Test============"
        ...
        print 'orginal data:'
        tmpList = r.content.decode('utf-8', 'replace')
        print tmpList
        #tempList = json.loads(r.content.decode('utf-8', 'replace'))['MemberList']
        del self.memberListTest
        self.memberListTest = tmpList
        print "==============Test====end========"
        print self.memberListTest
        return tmpList
    ...
    def send_msg(self, msg='Test Message', toUserName=None):
        ...
        print "======get msg response===content==========="
        print r.content
        print "======out end msg ========================="
        return r.content

    def send_msgTest(self, msg='Test Message',fromUserName=None, toUserName=None):
        url = '%s/webwxsendmsg' % self.loginInfo['url']
